...
struct SingleSpline <: QualityFunction
    N_knots::Int
    function SingleSpline(; N_knots::Int=5)
        new(N_knots)
    end
end
...
function (sqf::SingleSpline)(sp, parameters; check_bounds=false)
    ...
    # calculate S
    S = _SingleSpline(scaled_data, interval, sqf.N_knots, sp.errors_defined)

    return S
end


function _SingleSpline(scaled_data, interval, N_knots, errors_defined)::Float64
    ...
    try
        ...
        # knots = collect(range(xs[2], xs[end-1], length=N_knots))
        # spl = Spline1D(xs, ys, knots; w=ws)
        spl = Spline1D(xs, ys; w=ws, s=30.0)
        deltas = [(spl(xs[i]) - ys[i])^2 for i in eachindex(xs)]
        println(spl.fp)
        # w = inv.(es) .^ 2
        # invsumw = inv(sum(w))
        # return dot(xs, w) * invsumw
        return sum(deltas)
    catch
        return Inf
    end
    ...
    # @assert mean(deltas, weights(ws)) ≈ spl.fp
    # return mean(deltas, weights(ws))
    # try
    #     spl = Spline1D(xs, ys, knots; w=ws)
    #     residual = get_residual(spl)
